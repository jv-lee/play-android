 ...
 */
class CoinRecordAdapter(context: Context, data: List<Coin>) :
    ViewBindingAdapter<Coin>(context, data) {

    init {
        addItemStyles(CoinRecordItem())
    }

    inner class CoinRecordItem : ViewBindingItem<Coin>() {
        override fun getItemViewBinding(context: Context, parent: ViewGroup): ViewBinding {
            return ItemCoinRecordBinding.inflate(LayoutInflater.from(context), parent, false)
        }

        override fun convert(holder: ViewBindingHolder, entity: Coin, position: Int) {
