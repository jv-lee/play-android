 ...
 * @date 2020/3/30
 */
abstract class BaseNavigationFragment(layoutId: Int? = 0) : BaseFragment(layoutId) {
    ...
    private fun handleFragmentStop() {
        // stop事件在最开始更新生命周期状态-> home离开应用后所有fragment会回调onStop
        // 再次进入时不在最前台的fragment需要重新调用stop覆盖resume状态
